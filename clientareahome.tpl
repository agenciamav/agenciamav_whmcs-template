	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-6">
				...
				<address>
					<h3>{$clientsdetails.firstname} {$clientsdetails.lastname} {if $clientsdetails.companyname}({$clientsdetails.companyname}){/if}</h3>
			...
			<div class="col-md-6">
				...
				<ul class="list-unstyled">
					<li>
						<strong class="fa-2x btn-sm">{$clientsstats.productsnumactive}</strong> {$LANG.statsnumproducts} ({$clientsstats.productsnumtotal} no total)</span>
						<a href="clientarea.php?action=products" class="btn btn-sm btn-default pull-right text-uppercase"> {$LANG.view} <i class="fa fa-chevron-right"></i> </a>
					</li>
					<li>
						<strong class="fa-2x btn-sm">{ $clientsstats.numactivedomains}</strong> {$LANG.statsnumdomains} ({$clientsstats.numdomains} no total)
						<a href="clientarea.php?action=domains" class="btn btn-sm btn-default pull-right text-uppercase"> {$LANG.view} <i class="fa fa-chevron-right"></i> </a>
					</li>					
					<li>
						<strong class="fa-2x btn-sm">{ $clientsstats.numtickets }</strong> 
						{$LANG.statsnumtickets} 
						<a href="supporttickets.php" class="btn btn-sm btn-default pull-right text-uppercase"> {$LANG.view} <i class="fa fa-chevron-right"></i> </a>
					</li>
					<li>
						<strong class="fa-2x btn-sm">{ $clientsstats.numaffiliatesignups }</strong> 
						{$LANG.statsnumreferredsignups} 
						<a href="affiliates.php" class="btn btn-sm btn-default pull-right text-uppercase"> {$LANG.view} <i class="fa fa-chevron-right"></i> </a>
					</li>
					<li>
						{$LANG.creditcardyourinfo}: <strong>{if $defaultpaymentmethod}{$defaultpaymentmethod}{else}{$LANG.paymentmethoddefault}{/if}</strong> {if $clientsdetails.cctype}({$clientsdetails.cctype}-{$clientsdetails.cclastfour}) - <a href="clientarea.php?action=creditcard">Atualizar  <i class="fa fa-chevron-right"></i> </a>{/if}
					</li>
				</ul>
			</div>
		</div>
